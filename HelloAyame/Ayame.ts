...
export enum AyameConnectionState {
  NEW = 'new',
...
export class Ayame extends AyameEventTarget {
  ...
  async _createPeerConnection() {
    ...
    {
      logger.log('# Ayame: dataChannel : onopen : event =>', event);
    };
    dataChannel.onclose = (event: any) => {
      logger.log('# Ayame: dataChannel : onclose : event =>', event);
    };
  ...
  sendClickEvent(elementId: any)
  {
    let data = new DataView(new ArrayBuffer(3));
    const buttonClick = 4; // from Unity Render Streaming WebApp. register-events.js 
    data.setUint8(0, buttonClick);
